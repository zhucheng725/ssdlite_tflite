...
#from keras.applications.mobilenet import DepthwiseConv2D #for keras == 2.1.6
from tensorflow.keras.layers import DepthwiseConv2D # for kears == 2.2.4
...
def _inverted_res_block(inputs, expansion, stride, alpha, filters, stage=1, block_id=1, expand=True, output2=False):
    ...
    if expand:
        ...
        x = KL.Activation(relu6, name = name + '_expand_relu')(x)
    ...
    x = KL.Activation(relu6, name=name + '_dw_relu')(x)
...
def _followed_down_sample_block(inputs, conv_out_channel, sep_out_channel, id):
    ...
    x = KL.Activation(relu6, name=name + '_conv2_relu')(x)
    return x
...
def mobilenet_v2_ssdlite(input_image):
    ...
    x = KL.Activation(relu6, name='bbn_stage1_block1_relu')(x)
    ...
    link2 = KL.Activation(relu6, name= 'ssd_2_conv_relu')(x)
